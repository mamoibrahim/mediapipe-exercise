...
def recognise_latiralRaise(detection):
    ...
    try:
        ...
        # Lateral raise logic
        if (left_shoulder_angle > 75 and right_shoulder_angle > 75):
            if not range_flag:
                feedback = 'Yetersiz.'
            else:
                feedback = 'İyi tekrar!'
        ...
        elif (left_shoulder_angle > 30 and left_shoulder_angle < 80) and (right_shoulder_angle > 30 and right_shoulder_angle < 80) and state == 'up':
            range_flag = False
            feedback = ''
    except:
        left_angle.append(180)
...
def recognise_situp(detection):
    ...
    try:
        ...
        if angle_knee > 70:  # Triggers anytime the legs are not tucked in
            feedback = "Bacaklarınızı daha yakına sıkıştırın"
...
def recognise_legpress(detection):
    ...
    global state
    global feedback
    global range_flag , left_angle, right_angle
    global halfway
    try:
        landmarks = detection.pose_landmarks.landmark
...
with st.container():
    ...
    with col11:
        st.title(':blue[Egzersiz Analiz modeli 🤖]')
